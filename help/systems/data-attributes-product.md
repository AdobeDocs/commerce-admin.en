...
|`attribute_set_code`|Assigns the product to a specific attribute set or product template, according to product type. For example: `default`<br><br>After the product is created, the attribute set cannot be changed using the import functionality. However, you can change the attribute set from the Admin, and re-expor the product to update the CSV file .|
