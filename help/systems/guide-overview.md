...
| [Data transfer](data-transfer.md) | <ul><li>The import and export tools give you the ability to manage multiple records in a single operation. You can not only import new items, but also update, replace, and delete existing sets of products.</li><li>View data synchronization status for entities delivered to connected Commerce services from the [[!UICONTROL Data Management Dashboard]](data-dashboard.md).</li><li>Monitor the sync status for data feed export to Commerce SaaS services from the [[!UICONTROL Data Feed Sync Status]](data-feed-sync-status.md) page.</li></ul> |
