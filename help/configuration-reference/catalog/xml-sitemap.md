...
|[!UICONTROL Generation Method]|Store View|Determines how the XML sitemap is generated. `Standard` uses the traditional synchronous generation process and processes all data in memory, while `Batch` uses an asynchronous memory-optimized batch mode for greater flexibility and scalability. This option is available starting with the 2.4.9 release. Options: `Standard` / `Batch`|
