...
|New Order Status|Website|Determines the initial [order status](https://docs.magento.com/user-guide/sales/order-status.html) assigned to orders paid by a check or money order. Default value: Pending|
...
>[!NOTE]
>
>Do not select the _Not Capture_ option unless you are certain that you are going to capture the payment through Commerce later. You cannot create a credit memo until the payment has been captured using the Capture button.

## Purchase Order

![Purchase Order](./assets/payment-methods-purchase-order.png)<!-- zoom -->

<!-- Purchase Order_](../../stores-purchase/purchase-order.html) -->
...
|New Order Status|Website|Determines the initial [order status](https://docs.magento.com/user-guide/sales/order-status.html) assigned to orders paid by PO. Default value: Pending|
