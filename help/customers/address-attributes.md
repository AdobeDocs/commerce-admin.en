...
1. In the upper-right corner, click **[!UICONTROL Add New Attribute]**.
   ...
   ![Customer attribute properties](./assets/attribute-customer-address-new.png){width="600" zoomable="yes"}
...
1. To determine the type of input control that is used for data entry, set **[!UICONTROL Input Type]** to one of the following:
   ...
   - `Date` - Displays a date field with a pop-up calendar. Additional properties:  Use `Default Value` to specify the initial value of the field. <br/>Use `Minimal Value` to specify the earliest date that can be entered.  Use `Maximum Value` to specify the latest date that can be entered.
...
1. If the customer must enter a value in the field, set **[!UICONTROL Values Required]** to `Yes`.

1. To assign an initial value to the field, enter a **[!UICONTROL Default Value]**.

1. To check the data entered into the field for accuracy before the record is saved, set **[!UICONTROL Input Validation]** to the type of data to be allowed in the field. The available values depend on the _[!UICONTROL Input Type]_ specified.
   ...
   >[!NOTE]
   >
   >For system-defined customer address attributes—such as _Telephone_, _City_, and _Street_—Commerce enforces built-in server-side validation regardless of the **[!UICONTROL Input Validation]** setting. These default rules limit the allowed characters for each field (for example, telephone numbers can only contain digits, spaces, and certain symbols). The **[!UICONTROL Input Validation]** setting adds further restrictions but cannot override the built-in validation, which cannot be disabled through the Admin UI.

1. To apply a preprocessing filter to values entered in a text field, text area, or multiple line input type, set **[!UICONTROL Input/Output Filter]** to one of the following:
...
1. Complete the customer grid and segment properties:
...
1. Scroll down to the **[!UICONTROL Storefront Properties]** section.
   ...
   ![Customer address attributes - Storefront properties](./assets/attribute-customer-address-storefront-properties.png){width="600" zoomable="yes"}
...
1. Set **[!UICONTROL Forms to Use]** to each form that is to include the attribute.
...
1. When complete, click **[!UICONTROL Save Attribute]**.
   ...
   ![Customer address attributes - labels/options](./assets/attribute-customer-address-new-manage-label-options.png){width="600" zoomable="yes"}
...
|[!UICONTROL Input Validation]|The selection of options is determined by the input type. Options: <br/>**`None`** - The field has no input validation during data entry. <br/>**`Alphanumeric`** - Accepts any combination of numbers (0-9) and alphabetic characters (a-z, A-Z) during data entry. <br/>**`Alphanumeric with Space`** - Allows spaces in the street address to meet carrier maximum length requirements. During checkout, customers can enter letters (a-z, A-Z), numbers (0-9), and spaces in street addresses. Extra spaces are trimmed when the address is saved.<br/>**`Numeric Only`** - Accepts only numbers (0-9) during data entry. <br/>**`Alpha Only`** - Accepts only alphabetic characters (a-z, A-Z) during data entry. <br/>**URL** - Accepts only a URL during data entry. <br/>**`Email`** - Accepts only an email address during data entry. <br/>**`Length Only`** - Validates the input based on the length of the data entered into the field. <br/><br/>**Note:** For system-defined attributes such as _Telephone_, _City_, and _Street_, built-in server-side validation is always enforced in addition to any **[!UICONTROL Input Validation]** setting. These default rules restrict allowed characters for each field and cannot be overridden. The **[!UICONTROL Input Validation]** setting only adds further constraints.|
...
|[!UICONTROL Forms to Use in]|Determines the pages with data entry forms where the attribute appears. Options: <br/>[`Customer Address Registration`](account-create.md) <br/>[`Customer Account Address`](account-dashboard-address-book.md)|
