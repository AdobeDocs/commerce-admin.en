...
1. To check the data entered into the field for accuracy before the record is saved, set **[!UICONTROL Input Validation]** to the type of data to be allowed in the field. The available values depend on the _[!UICONTROL Input Type]_ specified.
   ...
   >[!NOTE]
   >
   >For system-defined customer address attributes—such as _Telephone_, _City_, and _Street_—additional server-side validation rules are enforced regardless of the **[!UICONTROL Input Validation]** setting. These built-in rules restrict the allowed characters for each field (for example, telephone numbers may only contain digits, spaces, and certain symbols). The **[!UICONTROL Input Validation]** setting acts as an additional constraint on top of these defaults and cannot override them. There is no way to disable the built-in server-side validation through the Admin UI.
