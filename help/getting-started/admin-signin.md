...
Additionally, consider [updating the Admin Base URL configuration](/help/configuration-reference/advanced/admin.md#admin-base-url) to change the default `/admin` endpoint to a custom path. Configuring a custom path provides the following security benefits:
...
>[!NOTE]
>
>If a breach is suspected, make sure to remove all unknown Admin users and reset all Admin passwords and review the [Security action plan](/help/systems/security.md) for further steps.
