...
- `dataservices_customer_id`: Indicates if a shopper has signed in or signed out. This cookie contain the customer's unique ID in the system.
- `dataservices_customer_group`: Indicates a customer's group. This cookie is stored as [sha1](https://en.wikipedia.org/wiki/SHA-1) checksum of customer's group ID.
- `dataservices_cart_id`: Identifies a shopper's cart actions. This cookie contains customer's unique cart ID in the system.
- `dataservices_product_context`: Identifies a shopper's product interactions. This cookie contains customer's unique quote ID in the system.
