...
Two-factor Authentication (TFA or 2FA) is an added layer of security to better protect your [!DNL Commerce] account from unauthorized access. To complete the log in process, TFA requires a _second factor_ in addition to the standard username and password credentials. This second factor takes the form of temporary verification codes that are continuously generated by a TFA application installed on your mobile device and paired with your [!DNL Commerce] account.

With TFA enabled, your account is more secure because an unauthorized user cannot log in unless they have both your username and password credentials (first factor) and the ability to retrieve a valid verification code from the TFA application on your personal device (second factor).
