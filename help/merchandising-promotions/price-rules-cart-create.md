...
|[!UICONTROL Discount Qty Step (Buy X)]|Sets the number of products represented by `X` in a `Buy X Get Y Free` promotion. Also, sets how many products must be added to the cart together in batches to apply "Fixed amount discount" and "Percent of product price discount" promotions.|
