...
Admin [permissions](../systems/permissions.md) can be set to restrict access to [tax resources](../systems/permissions-user-roles.md#role-resources), based on the business _need to know_. To create an Admin role with access to tax settings, choose both the Sales/Tax and System/Tax resources. If setting up a website for a region that differs from your default shipping point of origin, you must also allow access to the System/Shipping resources for the role. The shipping settings determine the store tax rate that is used for catalog prices.
