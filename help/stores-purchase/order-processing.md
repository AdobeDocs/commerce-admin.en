...
Review the following sections in the sales order, using the field descriptions for reference.### Process an order
...
>[!NOTE]
>
>An Admin user must have **Sales / Archive** [permissions](https://docs.magento.com/user-guide/system/permissions-user-roles.html) for their role scope to see the _Invoices_, _Credit Memos_, and _Shipments_ order tabs.
...
|Order Placed from Quote|![B2B for Adobe Commerce](../assets/b2b.svg) (Available with B2B for Adobe Commerce) Indicates the quote from which the order was generated, if applicable. The quote name is linked to the quote.|
...
1. If you are ready to invoice the order, click **Invoice** and follow the instructions to [create an invoice](invoices.md#create-an-invoice).
